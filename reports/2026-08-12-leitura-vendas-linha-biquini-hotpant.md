...
2. **Só demanda realizada.** A base mostra o que **vendeu**, não o catálogo completo —
   ...
   registrado na sessão de origem) ou o site (bloqueado por egress naquela sessão).
3. **URL de página de produto não está na base** e o site estava bloqueado por egress —
   as URLs finais dos anúncios ficaram **pendentes de verificação** (ver 00-produtos.md).
4. **Sem margem** na base — usa-se **preço** como proxy de valor de conversão e
   sinaliza-se a limitação, conforme decidido.
...
- **Volume de busca / CPC / concorrência** das keywords → Semrush BR ou Google Keyword
  Planner. **Bloqueio da sessão de origem** — reaberto nesta sessão (Semrush MCP disponível).
- **URLs finais, composição/material exatos, preço vigente e frete** → exige o site
  (bloqueado na origem) ou Nuvemshop (MCP ausente na origem).

---
*Proveniência herdada do HANDOFF da campanha (sessão de origem). A base `vendas.json`
original vive no repositório `zerohora-painel`; não foi reanexada a este repositório.*
